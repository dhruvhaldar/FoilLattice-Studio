...
import SolverSetupDialog from './components/SolverSetupDialog.jsx';
...
export default function App() {
  ...
  const [showSolverSetup, setShowSolverSetup] = useState(false);
  ...
  useEffect(() => {
    health().then(setEngine).catch(() => setEngine({ offline: true }));
    solverApi.getStatus().then((status) => {
      setSolverStatus(status);
      if (Object.values(status.solvers).some((item) => !item.installed || item.updateAvailable)) setShowSolverSetup(true);
    }).catch(() => {});
  }, []);
  ...
  return <div className="app-shell">
    <header className="topbar">
      ...
      <div className="top-actions">
        ...
        {solverStatus && <Button variant="outline-light" onClick={() => setShowSolverSetup(true)}><i className="fa-solid fa-microchip" /> Solvers</Button>}
    ...
    <SolverSetupDialog show={showSolverSetup} status={solverStatus} onDismiss={() => setShowSolverSetup(false)} onChanged={(status) => { setSolverStatus(status); health().then(setEngine).catch(() => {}); }} />
